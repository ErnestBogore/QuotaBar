<script lang="ts">
  ...
  import {
    checkForUpdates, deleteHistory, getState, hideMainWindow, installDesktopHook, installUpdate,
    removeDesktopHook, repairDesktopHook, requestOverride, updateSettings
  } from '$lib/api';
  ...
  async function saveSettings() {
    await run('settings', () => updateSettings({ allowanceWeeklyPoints: allowance, launchAtLogin, notificationsEnabled }));
  }
  ...
  async function checkUpdates() {
    ...
    try {
      ...
      updateMessage = 'Could not check right now. Try again later.';
    } finally {
      busy = '';
    }
  }
  ...
  function goBack() { settingsOpen = false; detailsOpen = false; passOpen = false; error = ''; }
</script>
...
<main class:locked={snapshot?.state === 'exhausted'}>
  <header>
    {#if settingsOpen || detailsOpen || passOpen}
      ...
      <strong class="view-title">{settingsOpen ? 'Settings' : detailsOpen ? 'How it works' : 'Emergency pass'}</strong>
    ...
    {:else}
      ...
      <div class="header-actions"><button class="icon-button" aria-label="Open settings" title="Settings" on:click={() => (settingsOpen = true)}><svg viewBox="0 0 20 20" aria-hidden="true"><path d="M10 6.8A3.2 3.2 0 1 0 10 13.2 3.2 3.2 0 0 0 10 6.8Z" /><path d="M16 11.1v-2.2l-1.8-.5a6 6 0 0 0-.5-1.1l.9-1.7L13 4l-1.7.9a6 6 0 0 0-1.1-.5L9.6 2.6H7.4l-.5 1.8a6 6 0 0 0-1.1.5L4.1 4 2.5 5.6l.9 1.7a6 6 0 0 0-.5 1.1l-1.8.5v2.2l1.8.5a6 6 0 0 0 .5 1.1l-.9 1.7L4.1 16l1.7-.9a6 6 0 0 0 1.1.5l.5 1.8h2.2l.5-1.8a6 6 0 0 0 1.1-.5l1.7.9 1.6-1.6-.9-1.7a6 6 0 0 0 .5-1.1l1.9-.5Z" /></svg></button><button class="icon-button" aria-label="Hide QuotaBar" title="Hide QuotaBar" on:click={hideMainWindow}><svg viewBox="0 0 20 20" aria-hidden="true"><path d="m5.5 5.5 9 9m0-9-9 9" /></svg></button></div>
    {/if}
  </header>
  ...
  {:else if settingsOpen}
    <section class="settings-section"><h2>General</h2><div class="settings-group">
      ...
      <label class="setting-row"><span><strong>Usage notifications</strong><small>Alerts at 50%, 75%, 90%, and 100%</small></span><input type="checkbox" bind:checked={notificationsEnabled} /></label>
    </div></section>
    ...
    <section class="settings-section"><h2>Updates</h2><div class="settings-group">
      <div class="gate-setting"><div><strong>{availableUpdate ? `QuotaBar ${availableUpdate}` : 'Keep QuotaBar current'}</strong><small>{updateMessage}</small></div>
        <button disabled={busy !== ''} on:click={availableUpdate ? applyUpdate : checkUpdates}>{availableUpdate ? 'Install' : busy === 'update' ? 'Checking…' : 'Check'}</button>
      </div>
    </div></section>
...
<style>
  ...
  .icon-button{width:30px;height:30px;display:grid;place-items:center;padding:0;color:#a9a9ae;border:0;border-radius:8px;background:transparent;cursor:pointer}.icon-button:hover{color:#fff;background:rgba(255,255,255,.07)}.icon-button svg{width:18px;fill:none;stroke:currentColor;stroke-width:1.45;stroke-linecap:round;stroke-linejoin:round}.view-title{font-size:13px;font-weight:650}.header-spacer{width:30px}
